...
function ProfileHeader({ user, displayName, userInitials }: { user: any; displayName: string; userInitials: string }) {
  return (
    <div className="flex items-center gap-6 mb-8">
      <div className="relative">
        {user?.profilePicture ? (
          <Image src={user.profilePicture} alt={displayName} width={80} height={80} className="rounded-full object-cover border-4 border-white shadow-lg" />
        ) : (
    ...
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <Card variant="elevated" className="p-8 text-center">
          ...
          <p className="text-slate-600 mb-6">Your profile is pending admin approval. You'll be notified once approved.</p>
        </Card>

        {/* Document uploads available even while pending */}
    ...
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <Card variant="elevated" className="p-8 text-center">
          ...
          <p className="text-slate-600">Your profile was not approved. Please contact support for more information.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <Card variant="elevated" className="p-8 mb-6">
        <div className="flex justify-between items-center mb-6">
          ...
          {profile && !editing && (
            <Button onClick={() => setEditing(true)} variant="secondary" size="sm">Edit Profile</Button>
          )}
        </div>
        ...
        {editing ? (
          <form onSubmit={handleSaveProfile} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Input label="Companies (comma-separated)" value={formData.companies} onChange={(e) => setFormData({ ...formData, companies: e.target.value })} placeholder="Google, Microsoft, Amazon" />
            ...
            <Input label="Roles Supported (comma-separated)" value={formData.rolesSupported} onChange={(e) => setFormData({ ...formData, rolesSupported: e.target.value })} placeholder="Software Engineer, Product Manager" required />
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Difficulty Levels</label>
              <div className="flex gap-3">
                {['EASY', 'MEDIUM', 'HARD'].map(level => (
                  <button key={level} type="button" onClick={() => toggleDifficulty(level)}
                    className={`px-4 py-2 rounded-lg border-2 transition-all ${formData.difficultyLevels.includes(level) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-700 border-slate-300 hover:border-indigo-400'}`}>
                    {level}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Session Types Offered</label>
              <div className="flex gap-3">
                {['GUIDANCE', 'INTERVIEW'].map(type => (
                  <button key={type} type="button" onClick={() => toggleSessionType(type)}
                    className={`px-4 py-2 rounded-lg border-2 transition-all ${formData.sessionTypesOffered.includes(type) ? 'bg-violet-600 text-white border-violet-600' : 'bg-white text-slate-700 border-slate-300 hover:border-violet-400'}`}>
                    {type}
                  </button>
                ))}
              </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div><p className="text-sm text-slate-500 mb-1">Companies</p><p className="font-medium text-slate-900">{profile.companies?.join(', ') || 'Not set'}</p></div>
            <div><p className="text-sm text-slate-500 mb-1">Experience</p><p className="font-medium text-slate-900">{profile.yearsOfExperience ? `${profile.yearsOfExperience} years` : 'Not set'}</p></div>
            <div><p className="text-sm text-slate-500 mb-1">Roles</p><p className="font-medium text-slate-900">{profile.rolesSupported?.join(', ') || 'Not set'}</p></div>
            <div><p className="text-sm text-slate-500 mb-1">Difficulty</p><p className="font-medium text-slate-900">{profile.difficultyLevels?.join(', ') || 'Not set'}</p></div>
            <div><p className="text-sm text-slate-500 mb-1">Session Types</p><p className="font-medium text-slate-900">{profile.sessionTypesOffered?.join(', ') || 'Not set'}</p></div>
            <div>
              <p className="text-sm text-slate-500 mb-1">LinkedIn</p>
              {profile.linkedinUrl ? (
                <a href={profile.linkedinUrl} target="_blank" rel="noopener noreferrer" className="font-medium text-violet-600 hover:text-violet-700 underline">View Profile →</a>
...
function DocumentUploadSection({ profile, resumeFile, idCardFile, setResumeFile, setIdCardFile, resumeInputRef, idCardInputRef, onUpload, uploading, error, success }: any) {
  return (
    <Card variant="elevated" className="p-8 mt-6">
      <h2 className="text-2xl font-display font-semibold text-slate-900 mb-2">Verification Documents</h2>
      <p className="text-slate-500 text-sm mb-6">Upload your resume and company ID card so the admin can verify your credentials before approving your profile.</p>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
          ...
          {profile?.resumeUrl && (
            <div className="flex items-center gap-2 mb-3 p-3 bg-green-50 border border-green-200 rounded-lg">
              ...
              <a href={profile.resumeUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-600 hover:underline ml-auto">View →</a>
            </div>
          )}
        ...
        <div>
          ...
          {profile?.idCardUrl && (
            <div className="flex items-center gap-2 mb-3 p-3 bg-green-50 border border-green-200 rounded-lg">
              <span className="text-green-600 text-sm">✅ Uploaded</span>
              <a href={profile.idCardUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-indigo-600 hover:underline ml-auto">View →</a>
            </div>
          )}
          <div className="border-2 border-dashed border-slate-300 rounded-xl p-4 hover:border-indigo-400 transition-colors">
            <input
      ...
      {success && <p className="mt-4 text-sm text-green-600 bg-green-50 border border-green-200 rounded-lg px-4 py-2">✅ {success}</p>}
